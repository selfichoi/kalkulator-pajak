...
export default function App() {
  ...
  // State untuk Tema (Default Dark Mode)
  const [isDarkMode, setIsDarkMode] = useState(true);

  // Efek untuk mengganti class 'dark' di elemen HTML paling atas
  useEffect(() => {
    if (isDarkMode) {
  ...
  const inputClass = "w-full bg-slate-50 dark:bg-slate-950 border border-slate-300 dark:border-slate-700 rounded-lg px-4 py-3 text-slate-900 dark:text-slate-200 focus:outline-none focus:ring-2 focus:ring-emerald-500 transition-all";

  return (
    <div className="min-h-screen flex flex-col font-sans transition-colors duration-300">
      <Header isLoggedIn={activePage !== 'login'} onLogout={handleLogout} isDarkMode={isDarkMode} toggleTheme={toggleTheme} />
          ...
          <div className="flex-grow flex items-center justify-center animate-in fade-in zoom-in-95 duration-500">
            <div className="bg-white dark:bg-slate-800 p-8 md:p-10 rounded-3xl shadow-2xl border border-slate-200 dark:border-slate-700/50 w-full max-w-md relative overflow-hidden transition-colors duration-300">
              ...
              <div className="text-center mb-8 relative z-10">
                <div className="w-24 h-24 mx-auto mb-4 overflow-hidden rounded-2xl shadow-lg border-2 border-emerald-500/50 bg-slate-100 dark:bg-slate-900">
                  <img src="/foto-kelompok.jpg" alt="Foto Kelompok 7" className="w-full h-full object-cover" />
                </div>
                <h2 className="text-2xl font-bold text-slate-900 dark:text-white tracking-tight">WELCOME</h2>
                <p className="text-slate-500 dark:text-slate-400 text-sm mt-2">Silahkan Masuk </p>
              </div>

              <form onSubmit={handleLogin} className="space-y-5 relative z-10">
                <div>
                  <label className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-2">Username</label>
                  <input type="text" className={inputClass} placeholder="Masukkan username" value={username} onChange={(e) => setUsername(e.target.value)} required />
                </div>
                <div>
                ...
                {loginError && (
                  <div className="bg-rose-50 dark:bg-rose-500/10 border border-rose-200 dark:border-rose-500/20 text-rose-600 dark:text-rose-400 text-sm px-4 py-3 rounded-lg text-center font-medium">
                    {loginError}
                  </div>
                )}

                <button type="submit" className="w-full mt-2 bg-emerald-600 hover:bg-emerald-500 text-white font-bold py-3.5 px-4 rounded-xl shadow-lg shadow-emerald-900/20 transition-all active:scale-[0.98]">
                  Masuk Sekarang
                </button>
              </form>
            </div>
          ...
          <div className="animate-in fade-in duration-500 pt-8">
            <div className="text-center mb-12">
              <h1 className="text-4xl md:text-5xl font-extrabold text-slate-900 dark:text-white mb-4 tracking-tight">
                Selamat Datang di <span className="text-transparent bg-clip-text bg-gradient-to-r from-emerald-500 to-cyan-500">PajakID</span>
              </h1>
          ...
          <div className="animate-in fade-in duration-500">
            ...
            <div className="mb-10">
              <h1 className="text-3xl md:text-4xl font-extrabold text-slate-900 dark:text-white mb-2 tracking-tight">
                Kalkulator <span className="text-transparent bg-clip-text bg-gradient-to-r from-emerald-500 to-cyan-500">PPN & PPnBM</span>
              </h1>
              <p className="text-slate-600 dark:text-slate-400">Pilih kategori barang/jasa dan masukkan nilai transaksi Anda.</p>
            </div>
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
              <div className="lg:col-span-7"><TaxForm onResult={setResult} onReset={() => setResult(null)} /></div>
              <div className="lg:col-span-5"><div className="sticky top-24"><ResultCard result={result} /></div></div>
            </div>
          </div>
        )}
