...
export default function TaxForm({ onResult, onReset }) {
  ...
  const cardClass = "bg-white dark:bg-slate-800 rounded-2xl p-6 shadow-xl border border-slate-200 dark:border-slate-700/50 transition-colors duration-300";
    ...
    <div className="space-y-6">
      <div className={cardClass}>
        ...
        <div className="space-y-4">
          <div>
            <label className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-2">Kelompok Kategori</label>
            <select className={inputClass} value={group} onChange={(e) => { setGroup(e.target.value); setSub(''); onReset(); }}>
              <option value="">— Pilih kelompok —</option>
              {Object.keys(taxData).map(key => <option key={key} value={key}>{taxData[key].label}</option>)}
            </select>
          </div>
          {group && (
            <div>
              <label className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-2">Sub-Kategori</label>
              <select className={inputClass} value={sub} onChange={(e) => { setSub(e.target.value); onReset(); }}>
                <option value="">— Pilih sub-kategori —</option>
                {taxData[group].items.map((item, idx) => <option key={idx} value={idx}>{item.n}</option>)}
              </select>
            </div>
          )}
          {selectedRule && (
             <div className="mt-4 p-4 bg-slate-50 dark:bg-slate-900/50 rounded-lg border border-slate-200 dark:border-slate-700">
               <p className="text-sm text-emerald-600 dark:text-emerald-400 font-semibold mb-1">Info Tarif:</p>
               <p className="text-xs text-slate-600 dark:text-slate-300">{selectedRule.note}</p>
      ...
      <div className={cardClass}>
        ...
        <div className="space-y-4">
          <div>
            <label className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-2">Metode Harga</label>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              <label className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-2">Harga Satuan (Rp)</label>
              <input type="number" className={inputClass} placeholder="0" value={price} onChange={(e) => { setPrice(e.target.value); onReset(); }} />
            </div>
            <div>
              <label className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-2">Kuantitas</label>
              <input type="number" className={inputClass} min="1" value={qty} onChange={(e) => { setQty(e.target.value); onReset(); }} />
            </div>
          </div>
          <button onClick={handleCalculate} className="w-full mt-6 bg-emerald-600 hover:bg-emerald-500 text-white font-bold py-3 px-4 rounded-xl shadow-lg shadow-emerald-900/20 transition-all flex justify-center items-center gap-2">
            Hitung Pajak
          </button>
